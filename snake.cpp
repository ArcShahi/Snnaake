...
{
     std::deque<Vec2> body;
     Vec2 direction{ -1, 0 };  // Starting direction: left
     int score{ 0 };
     int girth{ 10 };
};
...
Snake initializeSnake() {
     return Snake{
         std::deque<Vec2>{{60, 15}, {61, 15}, {62, 15}, {63, 15}, {64, 15},
                          {65, 15}, {66, 15}, {67, 15}, {68, 15}, {69, 15}},
         {-1, 0},  // Initial direction: left
         0, 10
     };
}
...
int main()
{
     ...
     while (!WindowShouldClose())
     {
          ...
          {
               Color color = (i == 0) ? (gameState == GameState::GAME_OVER ? BLUE : YELLOW) : (gameState == GameState::GAME_OVER ? BLACK : RED);
          ...
          DrawText("ArcShahi", 1000, 870, 20, RED);
